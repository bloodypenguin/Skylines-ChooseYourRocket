...
namespace ChooseYourRocket.Detour
{
    public class RocketLaunchAIDetour : EventAI
    {
        ...
        private static readonly MethodInfo Method1 =
            typeof(RocketLaunchAI).GetMethod("FindVehicles", BindingFlags.Instance | BindingFlags.NonPublic);

        private static readonly MethodInfo Method2 =
            typeof(RocketLaunchAI).GetMethod("SimulationStep", BindingFlags.Instance | BindingFlags.Public);

        private static readonly MethodInfo Detour1 =
            typeof(RocketLaunchAIDetour).GetMethod("FindVehicles", BindingFlags.Instance | BindingFlags.NonPublic);

        private static readonly MethodInfo Detour2 =
            typeof(RocketLaunchAIDetour).GetMethod("SimulationStep", BindingFlags.Instance | BindingFlags.Public);
        ...
        public override void SimulationStep(ushort eventID, ref EventData data)
        {
            ...
            bool flag1 = false;
            if ((data.m_flags & (EventData.Flags.Active | EventData.Flags.Expired | EventData.Flags.Completed |
                                 EventData.Flags.Disorganizing | EventData.Flags.Cancelled)) == EventData.Flags.None)
            {
                if ((data.m_flags & EventData.Flags.Ready) == EventData.Flags.None && this.m_resourceConsumption != 0)
                {
                    ...
                    if (data.m_building != (ushort)0 && num4 > num3)
                    {
                        int num5 = 100000;
                        ushort vehicleID3 = instance2.m_buildings.m_buffer[(int)data.m_building].m_guestVehicles;
                        int num6 = 0;
                        while (vehicleID3 != (ushort)0)
                        {
                            switch ((TransferManager.TransferReason)instance1.m_vehicles.m_buffer[(int)vehicleID3]
                                .m_transferType)
                            {
                                ...
                                case TransferManager.TransferReason.Petrol:
                                    int size;
                                    int max;
                                    instance1.m_vehicles.m_buffer[(int)vehicleID3].Info.m_vehicleAI.GetSize(vehicleID3,
                                        ref instance1.m_vehicles.m_buffer[(int)vehicleID3], out size, out max);
                        ...
                        if (num3 <= num5 - 8000)
                            Singleton<TransferManager>.instance.AddIncomingOffer(
                                (data.m_flags & EventData.Flags.Preparing) == EventData.Flags.None
                                    ? TransferManager.TransferReason.Coal
                                    : TransferManager.TransferReason.Petrol, new TransferManager.TransferOffer()
                                {
                ...
                if ((data.m_flags & (EventData.Flags.Preparing | EventData.Flags.Ready)) != EventData.Flags.None &&
                    data.m_building != (ushort)0)
                {
                    //next line: use configured crawler
                    if (vehicleID1 == (ushort)0 && crawlerVehicle != null)
                    {
                        Vector3 position = instance2.m_buildings.m_buffer[(int)data.m_building]
                            .CalculatePosition(this.m_rocketSpawnPosition);
                        //next line: use configured crawler
                        if (instance1.CreateVehicle(out vehicleID1,
                            ref Singleton<SimulationManager>.instance.m_randomizer, crawlerVehicle, position,
                            TransferManager.TransferReason.None, false, false))
                        {
                            //next line: use configured crawler
                            crawlerVehicle.m_vehicleAI.SetSource(vehicleID1,
                                ref instance1.m_vehicles.m_buffer[(int)vehicleID1], data.m_building);
                            //next line: use configured crawler
                            crawlerVehicle.m_vehicleAI.SetTarget(vehicleID1,
                                ref instance1.m_vehicles.m_buffer[(int)vehicleID1], data.m_building);
                        }
                    }

                    //next line: use configured rocket
                    if (vehicleID1 != (ushort)0 && vehicleID2 == (ushort)0 && rocketVehicle != null)
                    {
                        Vector3 position = instance2.m_buildings.m_buffer[(int)data.m_building]
                            .CalculatePosition(this.m_rocketSpawnPosition);
                        //next line: use configured rocket
                        if (instance1.CreateVehicle(out vehicleID2,
                            ref Singleton<SimulationManager>.instance.m_randomizer, rocketVehicle, position,
                            TransferManager.TransferReason.None, false, false))
                        {
                            //next line: use configured rocket
                            rocketVehicle.m_vehicleAI.SetSource(vehicleID2,
                                ref instance1.m_vehicles.m_buffer[(int)vehicleID2], data.m_building);
                            //next line: use configured rocket
                            rocketVehicle.m_vehicleAI.SetTarget(vehicleID2,
                                ref instance1.m_vehicles.m_buffer[(int)vehicleID2], data.m_building);
                            ushort lastVehicle = instance1.m_vehicles.m_buffer[(int)vehicleID1]
                                .GetLastVehicle(vehicleID1);
                            if (lastVehicle != (ushort)0)
                            {
            ...
            if (data.m_building != (ushort)0)
            {
                ushort num = instance2.m_buildings.m_buffer[(int)data.m_building].m_eventIndex;
                if ((int)num == (int)eventID)
                {
                    Notification.ProblemStruct problems =
                        instance2.m_buildings.m_buffer[(int)data.m_building].m_problems;
                    Notification.ProblemStruct problemStruct = Notification.RemoveProblems(problems,
                        (Notification.ProblemStruct)Notification.Problem1.NoResources);
                    if (flag1)
                    {
                        instance2.m_buildings.m_buffer[(int)data.m_building].m_incomingProblemTimer =
                            (byte)Mathf.Min((int)byte.MaxValue,
                                (int)instance2.m_buildings.m_buffer[(int)data.m_building].m_incomingProblemTimer + 1);
                        if (instance2.m_buildings.m_buffer[(int)data.m_building].m_incomingProblemTimer >= (byte)4)
                            problemStruct = Notification.AddProblems(problemStruct,
                                (Notification.ProblemStruct)Notification.Problem1.NoResources);
                    }
                    else
                    ...
                    if (problemStruct != problems)
                    {
                        ...
                        Singleton<BuildingManager>.instance.UpdateNotifications(data.m_building, problems,
                            problemStruct);
                    }

                    GuideController properties = Singleton<GuideManager>.instance.m_properties;
                    if (properties != null)
                        Singleton<BuildingManager>.instance.m_rocketLaunchSite.Activate(properties.m_rocketLaunchSite,
                            data.m_building);
                }

                if (num != (ushort)0 && (instance3.m_events.m_buffer[(int)num].m_flags & (EventData.Flags.Preparing |
                    EventData.Flags.Active | EventData.Flags.Completed | EventData.Flags.Cancelled |
                    EventData.Flags.Ready)) == EventData.Flags.None)
                ...
                {
                    Vector3 position = instance2.m_buildings.m_buffer[(int)data.m_building]
                        .CalculatePosition(this.m_doorPosition);
                    ...
                    //next line: use configured crawler
                    Vector3 b = lastFramePosition + Vector3.ClampMagnitude(targetPos0 - lastFramePosition,
                        crawlerVehicle.m_maxSpeed * 16f);
                    ...
                    if ((data.m_flags & EventData.Flags.Loading) == EventData.Flags.None)
                    {
                        ...
                        if (this.m_doorOpenEffect != null && (int)num == (int)eventID)
                        {
                            ...
                            EffectInfo.SpawnArea spawnArea = new EffectInfo.SpawnArea(
                                Singleton<BuildingManager>.instance.m_buildings.m_buffer[(int)data.m_building]
                                    .CalculatePosition(this.m_doorPosition), Vector3.up, 10f);
                            Singleton<EffectManager>.instance.DispatchEffect(this.m_doorOpenEffect, instance4,
                                spawnArea, Vector3.zero, 0.0f, 1f, Singleton<AudioManager>.instance.DefaultGroup);
                        }
                    }
                }
                else if ((data.m_flags & EventData.Flags.Loading) != EventData.Flags.None)
                {
                    ...
                    {
                        InstanceID instance5 = new InstanceID();
                        instance5.Event = eventID;
                        EffectInfo.SpawnArea spawnArea = new EffectInfo.SpawnArea(
                            Singleton<BuildingManager>.instance.m_buildings.m_buffer[(int)data.m_building]
                                .CalculatePosition(this.m_doorPosition), Vector3.up, 10f);
                        Singleton<EffectManager>.instance.DispatchEffect(this.m_doorCloseEffect, instance5, spawnArea,
